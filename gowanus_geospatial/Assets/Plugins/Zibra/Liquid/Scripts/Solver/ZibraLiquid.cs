...
using com.zibra.common.PresetAnalytics;
#endif
using System;
...
namespace com.zibra.liquid.Solver
{
    ...
    ///     </para>
    /// </remarks>
    [AddComponentMenu("Zibra Effects - Liquid/Zibra Liquid")]
...
#region Properties
        ...
        /// <summary>
        ///     Render target containing liquid depth when using downscale.
        /// </summary>
        /// <remarks>
        ///     <para>
        ///         Only used when <see cref="EnableDownscale"/> is enabled.
        ...
        /// </remarks>
        [NonSerialized]
        public RenderTexture UpscaleDepth;
        ...
        public CameraEvent CurrentInjectionPoint = CameraEvent.AfterSkybox;
...
#if UNITY_EDITOR
                ZibraLiquidAnalytics.TrackSimulationInitialization(this);
                ZibraEffectsPresetAnalytics.SendPresetAnalyticsData(new string[]
                {
                    AdvancedRenderParameters.PresetName,
                    SolverParameters.PresetName,
                    MaterialParameters.PresetName,
                });
#endif
            }
            catch (Exception)
...
#region Interop structures
        ...
        private CameraEvent ActiveInjectionPoint = CameraEvent.AfterSkybox;
        ...
        private bool UpdateNativeTextures(Camera cam, float renderPipelineRenderScale)
        {
            ...
            isGlobalTexturesDirty = CreateTexture(ref UpscaleColor, textureResolution, FilterMode.Point, 0,
                                    ...
                                    isGlobalTexturesDirty;
            isGlobalTexturesDirty = CreateTexture(ref UpscaleDepth, textureResolution, FilterMode.Point, 32,
                                                  RenderTextureFormat.Depth, false) ||
                                    isGlobalTexturesDirty;
                                          ...
                                          RenderTargetIdentifier? sourceDepthTexture = null, Rect? viewport = null)
        {
            ...
            if (sourceColorTexture == null)
            {
                ...
                cmdBuffer.SetGlobalTexture("ShadedLiquidDepth", UpscaleDepth);
            }
            else
            {
                cmdBuffer.SetGlobalTexture("ShadedLiquid", sourceColorTexture.Value);
                cmdBuffer.SetGlobalTexture("ShadedLiquidDepth", sourceDepthTexture.Value);
            }
                                  ...
                                  RenderTargetIdentifier? depthTargetParam = null, Rect? viewport = null)
        {
            ...
            if (EnableDownscale)
            {
                cmdBuffer.SetRenderTarget(UpscaleColor, UpscaleDepth);
                cmdBuffer.ClearRenderTarget(true, true, Color.clear);
            }
            else
            {
