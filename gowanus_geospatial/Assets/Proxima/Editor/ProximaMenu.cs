...
namespace Proxima.Editor
{
    ...
    internal class ProximaMenu : EditorWindow
    {
        ...
        internal static void Initialize()
        {
            ...
            {
                ProximaTrial.UpdateRemainingDays();
            }

            if (!ProximaTrial.IsTrial && !EditorApplication.isPlayingOrWillChangePlaymode && ProximaSurvey.ShouldAsk())
            {
        ...
        private void OnGUI()
        {
            ...
            GUILayout.BeginHorizontal(GUILayout.ExpandHeight(true));
            {
                GUILayout.BeginVertical();
                {
                    ...
                    LinkButton(ProximaTrial.IsTrial ? "Reviews" : "Write a Review", _review);
                    if (!_allFeaturesInstalled)
                    {
                        LinkButton("Upgrade to Pro", _website, _proStyle);
                    }

                    if (!ProximaTrial.IsTrial && !ProximaSurvey.Completed)
                    {
                        if (Button("Feedback"))
                        {
                            ProximaSurvey.ShowSurvey();
                ...
                GUILayout.BeginVertical();
                {
                    ...
                    {
                        if (ProximaTrial.IsTrial)
                        {
                            if (ProximaTrial.RemainingDays > 0)
                            {
                                var day = ProximaTrial.RemainingDays == 1 ? "day" : "days";
                                GUILayout.Label("You have " + ProximaTrial.RemainingDays + " " + day + " left on your trial.", _trialStyle);
                            }
                            else
                            {
                                GUILayout.Label("Your trial has expired. Proxima components will no longer update.", _errorStyle);
                            }

                            GUILayout.Label("You can upgrade the trial to a full license at any time without affecting your project.", _bodyStyle);

                            EditorGUILayout.Space();
                            LinkButton("Purchase Proxima", StoreLink);
                        }
                        else
                        {
