...
namespace UltimateCameraController.Cameras.Controllers
{
	...
	public class CameraController : MonoBehaviour
	{
		...
		private void LateUpdate()
		{
			...
			if (followTargetPosition /*&& targetObject.position.y > 0*/)
			{
				//We set the position the camera should move to, to the sum of the offset and the target's position
				newPosition = targetObject.position + _cameraOffset;
		...
		private void OrbitCamera()
		{
			...
			if (Input.GetMouseButton((int)mouseButton) && (Input.GetAxis("Mouse Y") * rotationSpeed > 0))
			{
